...
export default class TopicsDatalist extends React.Component {
  ...
  getSelectableTopics(topicId, cardId) {}
  ...
  render(props) {
    ...
    const topics =
      topicId === 0 || this.props.card_id ? db.getTopics() : db.getTopicsExcept(topicId);
...
export default class TopicsDatalist extends React.Component {
  render() {
    ...
    const topicList = topics.map(t => (
      <option key={t.id} value={t.name} topic_id={t.id}>
        {t.id}
      </option>
    ));
    ...
    return (
      <div>
        <label htmlFor="parent">Parent</label>
        <input type="text" list="topics" id="parent" />
        <datalist id="topics">{topicList}</datalist>
      </div>
    );
  }
}
